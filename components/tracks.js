...
const Tracks = ({ data }) => {
  ...
  return (
    <>
      ...
      <div className="w-11/12 bg-brown mx-auto p-4 shadow-lg flex flex-col md:flex-row items-center justify-between mb-5">
        <Box title="Top 5 Tracks">
          {data.slice(0, 5).map((item, index) => (
            <li key={item.index}>
              <h2>
                {index + 1}. {item.name} by {item.artists[0].name}
              </h2>
            </li>
          ))}
        </Box>
